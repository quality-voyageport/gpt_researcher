...
async def generate_report(query, context, agent_role_prompt, report_type, websocket, cfg, feedback, gsheet_prompt): # JD
    """
    ...
    JD starts here:
    """
    prompt_to_use = generate_prompt(question= query, context= context, gsheet_prompt= gsheet_prompt, report_format= cfg.report_format, total_words= cfg.total_words)  # (query, context, cfg.report_format, cfg.total_words, gsheet_prompt)
    if feedback != None and feedback != "":
        print('\t\t** JD: Feedback included')
        feedback = f"Considerations: {feedback}"
        prompt_to_use = prompt_to_use.replace('***', feedback)

    print(f'\n\n*** JD: This is the new prompt to use:\n{prompt_to_use}')
    """
    JD ends here.
    """
    try:
        report = await create_chat_completion(
            ...
            messages=[
                {"role": "system", "content": f"{agent_role_prompt}"},
                {"role": "user", "content": f"{prompt_to_use}"}],               # JD            ***
            temperature=0,
            llm_provider=cfg.llm_provider,
            stream=True,
            websocket=websocket,
